...
@Component
class EnablePluginHandler @Autowired constructor(private val pluginRepository: PluginRepository): EventHandler<EnablePluginEvent>("enable_plugin", EnablePluginEvent::class.java) {
    override fun onEvent(session: WebSocketSession, value: EnablePluginEvent): Mono<WebSocketMessage> {
        return pluginRepository.findById(value.serverId)
                .flatMap { pluginRepository.save(it.enabled(value.name)).then(Mono.empty()) }
    }
}